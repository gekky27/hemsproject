                            ...
                            <form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <select id="venues_id" name="venues_id" class="form-input" required>
                                            ...
                                            @foreach ($venues as $venue)
                                                <option value="{{ $venue->id }}"
                                                    {{ old('venues_id') == $venue->id ? 'selected' : '' }}>
                                                    {{ $venue->name }} (Capacity:
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" id="ticket_price" name="ticket_price" class="form-input"
                                            value="{{ old('ticket_price') }}" min="0" required>
                                        @error('ticket_price')
                                            <p class="input-error">{{ $message }}</p>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select id="status" name="status" class="form-input" required>
                                            ...
                                            <option value="soldout" {{ old('status') == 'soldout' ? 'selected' : '' }}>
                                                Sold Out</option>
                                        </select>
                                        @error('status')
                                            <p class="input-error">{{ $message }}</p>
                                ...
                                <div class="form-group mt-4">
                                    ...
                                    <div class="file-input">
                                        <img src="https://placehold.co/500x500/444/fff?text=Images" alt="Event Cover"
                                            class="file-input-preview" id="coverPreview">
                                        <input type="file" id="cover_image" name="cover_image" class="form-input"
                                            onchange="previewImage(this, 'coverPreview')">
                                    </div>
                                    <p class="text-xs text-gray-500 mt-1">Recommended size: 1200x628 pixels (JPG, PNG)</p>
                                    @error('cover_image')
                                        <p class="input-error">{{ $message }}</p>
                                    @enderror
                                </div>
